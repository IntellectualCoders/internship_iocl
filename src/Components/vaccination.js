      ...
      <TableRow className={classes.root}>
        ...
        {row.available_capacity < 30 ? (
          <TableCell align="right" style={{ color: "red" }}>
            {" "}
            <b>{row.available_capacity}</b>
          </TableCell>
        ) : row.available_capacity < 100 ? (
...
const Vaccination = () => {
  ...
  async function searchByPincode() {
    const res = await axios
      .get(
        `https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByPin?pincode=${pincode}&date=${todayDate}`
      )
      .then((res) => {
  ...
  useEffect(() => {
    ...
    const fetchapi2 = async () => {
      ...
      const res = await axios
        .get(
          `https://cdn-api.co-vin.in/api/v2/admin/location/districts/${currstateid}`
        )
        .then((res) => {
  ...
  async function searchByDistrict() {
    ...
    const res = await axios
      .get(
        `https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=${currdistrictid}&date=${today}`
      )
      .then((res) => {
        setHospitals(res.data.centers);
        console.log(res);
  ...
  return (
    <>
      <div className={classes.root}>
        <Grid className={classes.row} container spacing={3}>
          <Grid item xs={12}>
            ...
            <Typography
              ...
              style={{ color: "red", textAlign: "center" }}
            >
              You can <b>NOT</b> book vaccination appointment via this app.
              <br />
              Please visit{" "}
              <a style={{ color: "red" }} href="https://www.cowin.gov.in/home">
                Cowin.gov.in
              </a>
            </Typography>
          </Grid>
          ...
          {currhospital ? (
            <Grid xs={12} md={4} id="hospInfoGrid">
                      ...
                      <Table aria-label="collapsible table">
                        ...
                        <TableBody>
                          {currhospital.sessions.map((row) =>
                            ...
                            ) : minAge === "45+" && row.min_age_limit === 45 ? (
                              <Row key={row.name} row={row} />
                            ) : minAge === "All" ? (
                              <Row key={row.name} row={row} />
                            ) : (
        ...
        <BottomNavigation showLabelsfo className={classes.footer}>
          <Typography style={{ color: "white", marginTop: "10px" }}>
            Made with <FavoriteIcon style={{ color: "red" }} /> by{" "}
            <a
              href="https://github.com/IntellectualCoders/Covid-19-leads"
              style={{ color: "white" }}
            >
              Intellectual Coders
            </a>
