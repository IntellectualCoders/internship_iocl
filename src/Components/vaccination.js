...
function Row(props) {
 ...
 const Vaccination =() => {
  ...
  const [currhospital,setCurrHospital] = useState();
  const [todayDate, setTodayDate] = useState(null);
  const [alignment, setAlignment] = React.useState('left');

  const [district,setDistrict]=useState('Central Delhi');
  const [state,setState]=useState("Delhi");
  const[currstateid,setCurrstateid]=useState(9);
  const[currdistrictid,setCurrDistrictid]=useState(141);
  ...
  const handleAlignment = (event, newAlignment) => {
    ...
    useEffect( ()=>{
      ...
      const fetchapi2 = async () =>{
        ...
        .then(
              data=>{
              setStatesname(data.data.states);
              console.log(data);
        ...
        const res = await axios
        .get(`https://cdn-api.co-vin.in/api/v2/admin/location/districts/${currstateid}`)
        .then(
              res=>{
              setDistrictname(res.data.districts);
              console.log(res);
    ...
    function stateid(event){
      setCurrstateid(event);
      console.log(currstateid);

  }
  function districtid(event)
  {
      setCurrDistrictid(event);
      console.log(currdistrictid);
  }

  async  function searchByDistrict(){
    setCurrHospital(null);
    setHospitals(null);
      var today = new Date();
      var dd = today.getDate();
      ...
      today = dd+'-'+mm+'-'+yyyy;
    const res = await axios
    .get(`https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=${currdistrictid}&date=${today}`)
    .then(
          res=>{
            setHospitals(res.data.centers);
            console.log(res);
    ...
    >
       <ToggleButton className={ classes.button} onClick={()=>{searchByPincode()}}   value="left" aria-label="left aligned">
       Search By Pincode
        
      </ToggleButton>
      <ToggleButton className={classes.button}  value="center" onClick={()=>{searchByDistrict()}} aria-label="centered">
      Search By district
      ...
      >
       { statesname && statesname.map((state)=>{
            
            return(
              <MenuItem key={state.state_id} value={state.state_name} onClick={(event)=>{stateid(state.state_id)}}>{state && state.state_name}</MenuItem>
    ...
    <FormControl variant="outlined" className={classes.formControl}>
      ...
      >
       { districtname && districtname.map((district)=>{
            ...
            return(
              <MenuItem key={district.district_id} value={district.district_name} onClick={(event)=>{districtid(district.district_id)}}>{district && district.district_name}</MenuItem>
    ...
    <FormControl className={classes.formControl}>
     <InputLabel id="demo-simple-select-outlined-label">Age</InputLabel>
       ...
       <Button color="primary" variant="contained" className={classes.buttondistrict} onClick={()=>{searchByDistrict()}}>Search by District</Button>
     </FormControl>
    </> : 
    <>
      <FormControl className={classes.formControl}>
     ...
     <Button color="primary" variant="contained" className={classes.buttondistrict} onClick={()=>{searchByPincode()}}>Search by Pincode</Button>
